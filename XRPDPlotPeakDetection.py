...
path = r'C:\Users\Simon\Documents\Documents\Ref\xrd as'
...
for subdir, dirs, files in os.walk(path):
    ...
    for file in files:
        ...
        if filepath.endswith('.xy'): # -------------------File extension check 
...
for names in directories:
  ...
  # for j in range(1):
        ...
        ##Loading data/
        # dfb = pd.read_csv(name,header=None, skiprows=None, encoding='cp1252')
        dfb = pd.read_csv(name, skiprows=None, delimiter=' ', encoding='cp1252')


         ### Preprocessing
         # Baseline removal
         #Loads x axis
        dfa.loc[:,0] = dfb.iloc[:,0]
  ...
  #
        ...
        peaks2, p2H_dict = find_peaks(dfanorm, distance=10, prominence=0.1, height=0.1, width=1) #--------------------------------------
  ...
  ax1.legend(reversed(handles), reversed(labels), loc='upper right', fontsize=fontsizeAxe*0.8)
